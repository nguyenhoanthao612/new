...
export default function AdminPanel() {
  ...
  const [activeTab, setActiveTab] = useState<"users" | "stats" | "questions" | "records" | "testSets" | "students">("users");
  ...
  // Allowed Students Form & Filter States
  ...
  const [studentIdToConfirmDelete, setStudentIdToConfirmDelete] = useState<string | null>(null);
  ...
  return (
    <div className="space-y-8 animate-fade-in" id="admin-panel-stage">
      ...
      >
        ...
        >
          ...
          <button
            ...
            className={`px-3 py-2 text-xs font-bold rounded-lg transition-all duration-200 flex items-center gap-1.5 ${activeTab === "students" ? "bg-purple-600 text-white shadow-lg shadow-purple-900/30" : "text-slate-400 hover:text-slate-200"}`}
          >
            <Users className="w-3.5 h-3.5" />
            Danh sách học sinh ({(allowedStudents || []).length})
          </button>
        </div>
      </div>
      ...
      {activeTab === "students" && (
        <div className="space-y-6 text-left animate-fade-in" id="admin-students-management">
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-50 pb-3 gap-3">
              <div>
                <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-2">
                  <span className="p-1 px-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-mono">BƯỚC 1</span>
                  {editingStudent ? "Hiệu Chỉnh Thông Tin Học Sinh" : "Thêm Học Sinh Vào Danh Sách"}
                </h3>
                <p className="text-[10px] text-slate-400 font-semibold font-mono">QUẢN LÝ MẬT KHẨU PHÒNG THI</p>
              </div>
              {editingStudent && (
                <button
                  onClick={() => {
                  ...
                  className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-[10px] font-black cursor-pointer transition"
                >
                  Hủy Chỉnh Sửa
                </button>
              )}
            ...
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                const name = formStudentName.trim();
                const cls = formStudentClass.trim();
                const pwd = formStudentPassword.trim();
                if (!name || !cls || !pwd) {
                  setAdminNotice({ type: "error", text: "Vui lòng nhập đầy đủ thông tin: Họ tên, lớp, và mật khẩu!" });
                  return;
                }
                setIsSubmittingStudent(true);
                try {
                  if (editingStudent) {
                    await updateAllowedStudent(editingStudent.id, name, cls, pwd);
                    setAdminNotice({ type: "success", text: "Cập nhật học sinh thành công!" });
                    setEditingStudent(null);
                  } else {
                    await addAllowedStudent(name, cls, pwd);
                    setAdminNotice({ type: "success", text: "Thêm học sinh thành công!" });
                  }
                  setFormStudentName("");
                  setFormStudentClass("");
                  setFormStudentPassword("");
                } catch (err: any) {
                  setAdminNotice({ type: "error", text: err.message || "Có lỗi xảy ra!" });
                } finally {
                  setIsSubmittingStudent(false);
                }
              }}
              className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end text-slate-800 text-xs"
            >
              <div className="space-y-1.5 text-left">
                <label className="block font-bold text-slate-700">Họ và Tên Học Sinh <span className="text-rose-500">*</span></label>
                <input
                  type="text"
                  placeholder="Ví dụ: Nguyễn Văn A"
                  value={formStudentName}
                  onChange={(e) => setFormStudentName(e.target.value)}
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition font-medium"
                />
              </div>

              <div className="space-y-1.5 text-left">
                <label className="block font-bold text-slate-700">Lớp <span className="text-rose-500">*</span></label>
                <input
                  type="text"
              ...
              <div className="space-y-1.5 text-left">
                <label className="block font-bold text-slate-700">Mật khẩu mới <span className="text-rose-500">*</span></label>
                <input
                  type="text"
                  placeholder="Mật khẩu vào thi"
                  value={formStudentPassword}
                  onChange={(e) => setFormStudentPassword(e.target.value)}
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition font-bold font-mono text-indigo-700"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-xl transition duration-150 shadow-lg cursor-pointer flex items-center justify-center gap-1.5"
              >
                {editingStudent ? <Edit className="w-3.5 h-3.5" /> : <Plus className="w-4 h-4" />}
                {isSubmittingStudent ? "Đang xử lý..." : editingStudent ? "Cập Nhật" : "Thêm Học Sinh"}
              </button>
            </form>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-50 pb-3 gap-3">
              <div>
                <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-2">
                  <span className="p-1 px-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-mono">BƯỚC 2</span>
                  Danh Sách Thí Sinh Được Phép Luyện Tập
                </h3>
                <p className="text-[10px] text-slate-400 font-semibold font-mono">STUDENT CREDENTIAL ROSTER</p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs w-full sm:w-auto">
                <input
                  ...
                  className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs outline-none focus:bg-white font-medium w-full sm:w-36 text-slate-800"
                />
                <select
                  value={studentClassFilter}
                  onChange={(e) => setStudentClassFilter(e.target.value)}
                  className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold outline-none cursor-pointer text-slate-800"
                >
                  <option value="all">Tất cả lớp</option>
                  {Array.from(new Set((allowedStudents || []).map((s) => s.className)))
                    .filter(Boolean)
                    .sort()
                    .map((cls) => (
            ...
            {!(allowedStudents && allowedStudents.length > 0) ? (
              <div className="p-12 text-center text-slate-400 text-xs italic">
                Chưa có dữ liệu học sinh được cấp quyền ôn luyện. Hãy nhập học sinh ở mẫu trên!
              </div>
            ) : (
              <div className="overflow-x-auto text-xs">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50 text-slate-600 font-black">
                      <th className="p-3 w-12 text-center">STT</th>
                      <th className="p-3">Họ và Tên</th>
                      <th className="p-3">Lớp học</th>
                      <th className="p-3">Mật khẩu của học sinh</th>
                  ...
                  <tbody>
                    {(allowedStudents || [])
                      .filter((s) => {
                        const matchQ = s.fullName.toLowerCase().includes(studentSearch.toLowerCase());
                        const matchC = studentClassFilter === "all" || s.className === studentClassFilter;
                        return matchQ && matchC;
                      })
                      .map((student, idx) => (
                        <tr key={student.id} className="border-b border-slate-50 hover:bg-slate-50/50 font-medium">
                          ...
                          <td className="p-3 font-semibold text-slate-600 uppercase font-mono">{student.className}</td>
                          <td className="p-3">
                            <span className="px-2 py-1 bg-slate-100 border border-slate-200 text-slate-800 rounded font-black font-mono tracking-wider">
                              {student.password}
                            </span>
                          </td>
                          <td className="p-3">
                            <div className="flex gap-2 justify-center">
                              <button
                                onClick={() => {
                                  ...
                                  setFormStudentName(student.fullName);
                                  setFormStudentClass(student.className);
                                  setFormStudentPassword(student.password || "");
                                  // Scroll to top of Step 1 form smoothly
                                  document.getElementById("admin-students-management")?.scrollIntoView({ behavior: "smooth" });
                                }}
                                className="px-2.5 py-1.5 border border-indigo-100 bg-indigo-50 hover:bg-indigo-600 hover:text-white rounded-lg text-[10px] font-bold text-indigo-700 transition cursor-pointer"
                              >
                                Sửa
                              </button>
                              <button
                                onClick={async () => {
                                  if (studentIdToConfirmDelete === student.id) {
                                    try {
                                      await deleteAllowedStudent(student.id);
                                      setAdminNotice({ type: "success", text: "Xóa học sinh thành công!" });
                                      setStudentIdToConfirmDelete(null);
                                    } catch (err: any) {
                                      setAdminNotice({ type: "error", text: err.message || "Lỗi xóa học sinh!" });
                                    }
                                  } else {
                                    setStudentIdToConfirmDelete(student.id);
                                    setTimeout(() => setStudentIdToConfirmDelete(null), 4000); // reset
                                  }
                                }}
                                className="px-2.5 py-1.5 border border-rose-100 bg-rose-50 hover:bg-rose-600 hover:text-white rounded-lg text-[10px] font-bold text-rose-700 transition cursor-pointer"
                              >
                                {studentIdToConfirmDelete === student.id ? "Xác nhận?" : "Xóa"}
                              </button>
                            </div>
                          </td>
                        </tr>
